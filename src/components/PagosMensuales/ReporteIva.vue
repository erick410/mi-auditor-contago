        ...
        <q-table table-class="primary lighten-5" title="Reporte ISR" :data="dataComprobantes" :columns="columns"
            ...
            <template v-slot:body="props">
                <q-tr :props="props" :class="'clase-total-' + props.row.año">
                    ...
                    <q-td v-if="token.rfc.length == 12" key="ivaRetenidoAnterior" :props="props">{{
                        FormatCurrency(props.row.ivaRetenidoAnterior)
                        }}</q-td>
        ...
        async GetReporteIva(ivaCargo) {
            try {
                ...
                for (let x = 0; x < this.selectedMes.value; x++) {
                    ...
                    const mesActual = ivaCargo[x].mes?.toUpperCase()
                    ObjIva.ivaRetenido = ivaRetenido.find(item => item.mes?.toUpperCase() === mesActual)?.importeIva || 0
        ...
        async GetReporteIvaFisicas(ivaCargo) {
            try {
                ...
                for (let x = 0; x < this.selectedMes.value; x++) {
                    ...
                    let mesActual = ivaCargo[x].mes?.toUpperCase()
