<template>
  <div class="q-pa-md">
    ...
    <q-table
      :data="items.detallesConstancia"
      :columns="columns"
      row-key="uuid"
      :rows-per-page-options="[10]"
      title="Detalles Constancias"
      :filter="filter"
    >
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td key="fechaTimbrado" :props="props">{{
            formatDate(props.row.fechaTimbrado)
          }}</q-td>
          <q-td key="rfcEmisor" :props="props">{{ props.row.rfcEmisor }}</q-td>
          <q-td key="nomEmisor" :props="props">{{ props.row.nomEmisor }}</q-td>
          <q-td key="rfcReceptor" :props="props">{{
            props.row.rfcReceptor
          }}</q-td>
          <q-td key="nomReceptor" :props="props">{{
            props.row.nomReceptor
          }}</q-td>
          <q-td key="tipoPago" :props="props">{{ props.row.tipoPago }}</q-td>
          <q-td key="baseRet" :props="props">{{
            formatCurrency(props.row.baseRet)
          }}</q-td>
...
export default {
  ...
  computed: {
    ...
    items() {
      return (
        this.$store.state.detallesIsrMStore || { cabecera: "", detalles: [], detallesConstancia:[] }
      );
    },
  },
  ...
  methods: {
    ...
    async descargarPdf(row) {
      ...
      try {
        const res = await axios.get(
          this.rutaAxios +
            "Retenciones/GetRetencionesDocumentos/erp_" +
            this.token.rfc +
            "/" +
            this.token.rfc +
            "?uuid=" +
        ...
        if (res.data && res.data.length > 0) {
          await generarPdfRetencion(
            res.data[0],
            require("@/assets/logo_contago_sin_fondo.png")
          );
        }
      } catch (e) {
    ...
    async descargarXml(row) {
      try {
        const res = await axios.get(
          this.rutaAxios +
            "Retenciones/GetXmlRetencion/erp_" +
            this.token.rfc +
            "/" +
            this.token.rfc +
            "/" +
