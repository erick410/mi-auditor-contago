...
export default {
    ...
    methods: {
        ...
        async GetReporte() {
            ...
            try {
                let response = await axios.get(this.rutaAxios + 'Gastos/GetReporteConceptosDescripcionAsync/erp_' + this.token.rfc + '/' + fI + '/' + fF + '/' + this.descripcion);
                let x = response.data;
        ...
        ExportExcel() {
            ...
            xlsx.writeFile(
                workbook,
                rfc + ' - ' + empresa +  ' - CONCEPTOS FACTURADOS DEL ' + periodo.toUpperCase() + ' POR DESCRIPCIÓN.xlsx'
            );
            },
