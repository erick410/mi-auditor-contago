<template>
    <div class="q-pa-md">
        ...
        <q-table :data="dataComprobantes" :columns="columns" row-key="rfc" :rows-per-page-options="[13]"
            class="my-sticky-column-table  shadow-0">
            <template v-slot:top>
                <span class="text-body1" content-style="font-size: 20px">Comprobantes Recibidos por Periodo </span>
                <q-space />
    ...
    methods: {
        ...
        async GetDetalle(item){
            ...
            try{
                ...
                this.$store.state.detallesFacturacionStore.cabecera = 'GASTOS ' + item.mes.toUpperCase() + ' DEL ' + this.selectedAnio;
        ...
        //     XLSX.writeFile(workbook, 'GASTOS FACTURADOS DE ' + this.selectedMesI.label + ' A ' + this.selectedMesF.label + ' ' + this.selectedAnio + '.xlsx');
        // },

        ExportExcel() {
            let reporte = 'REPORTE DE GASTOS FACTURADOS POR PERIODO'
            ...
            xlsx.writeFile(
                workbook,
                rfc + ' - ' + empresa +  ' - REPORTE DE GASTOS FACTURADOS DEL PERIODO ' + periodo.toUpperCase() + '.xlsx'
            );
            },
