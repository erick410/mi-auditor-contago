<template>
    <div class="q-pa-md">
        ...
        <q-table title="Reporte ISR" :data="dataComprobantes" :columns="columns" row-key="folioFiscal"
            ...
            <template v-slot:top>
                <span class="text-body1" content-style="font-size: 20px">Cuentas Por Pagar</span>
                <q-space />
        ...
        methods: {
            ...
            async GenerarGrafica(data) {
                ...
                data.forEach(function (item) {
                    ...
                    sumasPorConcepto[item.nombre] += item.porPagar;
                });

                var listaSumasPorConcepto = [];
                for (var concepto in sumasPorConcepto) {
                    listaSumasPorConcepto.push({ nombre: concepto, porPagar: sumasPorConcepto[concepto] });
                }

                let lista = listaSumasPorConcepto
                lista.sort((a, b) => b.porPagar - a.porPagar);

                // Tomar los primeros 10 elementos
                let primerosDiez = lista.slice(0, 10);

                const Conceptos = primerosDiez.map((item) => item.nombre);
                const importe = primerosDiez.map((item) => item.porPagar);

                let obj1 = {
                    type: 'line',
                    label: 'Por Pagar (Linea)',
                    borderColor: '#66BB6A',
                    borderWidth: 2,
                    fill: false,
                    data: importe
            ...
            //     XLSX.writeFile(workbook, 'CUENTAS POR PAGAR  DEL ' + fI + ' AL ' + fF + '.xlsx');
            // },
            ...
            let reporte = 'REPORTE DE CUENTAS POR PAGAR'
            ...
            xlsx.writeFile(
                workbook,
                rfc + ' - ' + empresa +  ' - REPORTE DE CUENTAS POR PAGAR DEL ' + periodo.toUpperCase() + '.xlsx'
            );
            },
