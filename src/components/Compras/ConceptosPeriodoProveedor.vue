<template>
    <div class="q-pa-md">
        ...
        <div class="row no-wrap items-center q-mt-md q-pa-sm">
            ...
            <q-select v-model="selectClientes" use-input input-debounce="0" label="Cliente" :options="optionsClientes"
        ...
        <q-table title="Reporte ISR" :data="dataComprobantes" :columns="columns" row-key="folioFiscal"
            ...
            <template v-slot:top>
                <span class="text-body1" content-style="font-size: 20px">Conceptos por Proveedor</span>
                <q-space />
...
export default {
    ...
    methods: {
        ...
        async GetReporte() {
            ...
            try {
                let response = await axios.get(this.rutaAxios + 'Gastos/GetReporteConceptosProveedorAsync/erp_' + this.token.rfc + '/' + fI + '/' + fF + '/' + this.selectClientes.value);
                let x = response.data;
...
const ordenados = [...objetos].sort((a, b) => b.importe - a.importe);

const todos = ordenados.slice(0, 10);
const importe = todos.map((item) => item.importe);

const sumaIngresos = this.dataComprobantes.reduce((acumulador, actual) => acumulador + actual.importe, 0);
const sumaTotal = sumaIngresos

let ObjIngresos = {
    label: "Importe: " + this.FormatoMiles(sumaIngresos),
...
let ObjIngresosLinea = {
    label: "Ingresos (Linea): " + this.FormatoMiles(sumaIngresos),
    type: 'line',
    borderColor: "rgba(0, 163, 92)",
    borderWidth: 1,
    data: importe,
...
let descripcion = []
for (let a of todos) {
    descripcion.push(a.descripcion)
}
...
this.charTitleE = 'Conceptos por RFC: ' + this.FormatoMiles(sumaTotal)
},
        async GetClientes(ObjFechaI, ObjFechaF) {
            this.loadingClientes = true;
            try {
                let response = await axios.get(this.rutaAxios + 'Gastos/GetProveedoresTodosReporteAsync/erp_' + this.token.rfc + '/' + ObjFechaI + '/' + ObjFechaF);
                let x = response.data;
                // console.log(x);
                this.options = [...x]
        ...
        //     XLSX.writeFile(workbook, 'CONCEPTOS FACTURADOS DEL ' + fI + ' AL ' + fF + ' POR PROVEEDOR.xlsx');
        // },
        ExportExcel() {
            let reporte = 'REPORTE DE CONCEPTOS FACTURADOS POR PROVEEDOR'
            ...
            xlsx.writeFile(
                workbook,
                rfc + ' - ' + empresa +  ' - REPORTE DE CONCEPTOS FACTURADOS POR PROVEEDOR DEL ' + periodo.toUpperCase() + '.xlsx'
            );
            },
        async UltimoDiaMes() {
