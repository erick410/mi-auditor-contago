<template>
    <div class="q-pa-md">
        ...
        <q-table title="Reporte ISR" :data="dataComprobantes" :columns="columns" row-key="dia" :rows-per-page-options="[32]"
            class="my-sticky-column-table  shadow-0 my-sticky-header-table">
            <template v-slot:top>
                <span class="text-body1" content-style="font-size: 20px">Comprobantes Recibidos por Mes</span>
                <q-space />
    ...
    methods: {
        async GetReporte() {
            try {
                ...
                let response = await axios.get(this.rutaAxios + 'Gastos/GetReporteMesAsync/erp_' + this.token.rfc + '/' + fechaI + '/' + fechaF);
                let x = response.data
        ...
        VerDetalles(item) {
            ...
            this.$store.state.detallesFacturacionStore.cabecera = 'GASTOS ' + item.dia + ' DE ' + this.selectedMes.label + ' DEL ' + this.selectedAnio;
        ...
        //     xlsx.writeFile(workbook, 'GASTOS FACTURADOS ' + this.selectedMes.label + ' ' + this.selectedAnio + '.xlsx');
        // },

        ExportExcel() {
            let reporte = 'REPORTE DE GASTOS FACTURADOS'
            ...
            xlsx.writeFile(
                workbook,
                rfc + ' - ' + empresa +  ' - REPORTE DE GASTOS FACTURADOS DE ' + this.selectedMes.label + ' DEL ' + this.selectedAnio + '.xlsx'
            );
            },
