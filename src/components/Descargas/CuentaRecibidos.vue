  ...
  <div class="ce-root">
    ...
    <div class="ce-filters">
      <div style="display: flex; align-items: flex-end; gap: 8px; flex-wrap: wrap">
        <div class="row">
          <div class="ce-title">Comprobantes Recibidos</div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async GetReporte() {
      ...
      try {
        const { data } = await axios.get(
          `${this.rutaAxios}Comprobante/GetCuentaComprobantesRecibidosAsync/erp_${this.token.rfc}/${this.selectedAnio}`
        );
        for (const a of data) {
    ...
    async GetConciliaSat(año, mes) {
      try {
        const { data } = await axios.get(
          `${this.rutaDescargas}Descargas/GetConcilacionSatAsync/${this.token.rfc}/${año}/${mes}/Recibidos`
        );
    ...
    async GetConciliacionSat(mes, año, fI, fF) {
      try {
        const { data } = await axios.get(
          `https://api-scrapper-a.contago.com.mx/Metadata/${fI}/${fF}/Recibidos/${this.token.rfc}`
        );
