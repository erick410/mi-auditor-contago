    ...
    <div class="q-pa-md">
        ...
        <q-dialog v-model="dialogConciliaSat" transition-show="scale" transition-hide="scale">
            <q-card>
                ...
                <q-card-section>
                    <q-table :pagination.sync="pagination" :data="itemsConciliaSat" :columns="columnsConciliaSat" row-key="tipo" :rows-per-page-options="[0]" flat>
        ...
        <div class="row no-wrap items-center q-mt-md q-pa-sm">
            <q-space />
            <q-select outlined dense v-model="selectedMes" :options="itemsMes" label="Mes a Conciliar" style="width:170px"
                class="q-mr-xs" />
            <q-btn push color="green-9" @click="Conciliar()" icon="mdi-code-equal" rounded flat size="18px" padding="xs">
                <q-tooltip transition-show="flip-right" transition-hide="flip-left" content-style="font-size: 14px"
                    :offset="[10, 10]">Conciliar Mes</q-tooltip>
            </q-btn>
            <q-space />
        </div>
        <q-table title="Comprobantes descargados y almacenados (Recibidos)" :pagination.sync="pagination"
...
export default {
    ...
    methods: {
        async GetReporte() {
            ...
            this.itemsDescargas = [];
            try {
                let response = await axios.get(this.rutaAxios + 'Comprobante/GetCuentaComprobantesRecibidosAsync/erp_' + this.token.rfc + '/' + this.selectedAnio);
                let x = response.data;
                this.dataResult = x;
                this.dialog = false;
                this.charTitleE = 'Recibidos'
                // console.log(x)
                for (let a of x) {
        ...
        async GetGrafica() {
            ...
            const sumaTotal = sumaIngresos + sumaNotas + sumaPagos;
        ...
        async ConciliaSatCancelados(item){
            this.$q.loading.show({ spinner: QSpinnerCube, spinnerColor: 'purple', spinnerSize: 140, message: 'Consultando cancelados con la platadorma del SAT...', messageColor: 'white' })
                ...
                console.log(fI, fF)
                try{
                    let response = await axios.get('https://api-scrapper-a.contago.com.mx/Cancelaciones/' + fI + '/' + fF + '/Recibidos/' + this.token.rfc);
                    this.$q.loading.hide();
        ...
        async GetConciliacionSat(mes, año, fI, fF){
            try{
                let items = [];
                // let response = await axios.get('http://localhost:8080/Metadata/' + fI + '/' + fF + '/Recibidos/' + this.token.rfc);
                let response = await axios.get('https://api-scrapper-a.contago.com.mx/Metadata/' + fI + '/' + fF + '/Recibidos/' + this.token.rfc);
