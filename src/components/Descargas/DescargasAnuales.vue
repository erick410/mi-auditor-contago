        ...
        <div class="dsc-filters">
            <div class="dsc-filters__row">
                <div class="dsc-header-info">
                    ...
                    <div>
                        <div class="dsc-header-title">Declaración Anual</div>
                ...
                <div class="dsc-field dsc-field--sm">
                    ...
                    <q-select dense outlined v-model="ejercicio" :options="catEjercicios" class="dsc-input" />
                </div>

                <!-- Formato -->
                <div class="dsc-field dsc-field--sm">
                    <div class="dsc-field__label">Formato</div>
                    <q-select dense outlined v-model="formato" :options="catFormatos" option-label="label"
                        option-value="value" emit-value map-options class="dsc-input" />
                </div>

                <div class="dsc-field dsc-field--btns">
                    <q-btn dense unelevated color="primary" icon="mdi-cloud-download" label="Solicitar"
                        :loading="cargandoSolicitud" :disable="cargandoSolicitud || !ejercicio || !formato"
                        class="dsc-btn" @click="solicitarDescarga" />
                    <q-btn dense unelevated outline color="primary" icon="mdi-refresh" label="Actualizar"
                        :loading="cargandoHistorial" :disable="cargandoHistorial" class="dsc-btn"
            ...
            <q-table dense flat bordered :data="registrosFiltrados" :columns="columns" :filter="filter"
                ...
                <template v-slot:top>
                    <div class="dsc-table__top">
                        <span class="dsc-table__title">Historial Declaración Anual</span>
                ...
                <template v-slot:body="props">
                    <q-tr :props="props" :class="rowClass(props.row)">
                        ...
                        <q-td key="ejercicio" :props="props" class="dsc-td text-center">{{ props.row.ejercicio }}</q-td>
                        <q-td key="formato" :props="props" class="dsc-td text-center">
                            <span class="dsc-pill dsc-pill--blue">{{ (props.row.formato || '').toUpperCase() }}</span>
                        </q-td>
                        ...
                        <q-td key="nombreArchivo" :props="props" class="dsc-td">{{ props.row.nombreArchivo || '—'
                            }}</q-td>

                        <q-td key="acciones" :props="props" class="dsc-td" auto-width>
                            <div class="dsc-actions">
                                <q-btn v-if="props.row.estado === 'EN_PROCESO'" dense round flat size="md"
                                    color="blue-6" icon="mdi-loading" disable loading>
                                    <q-tooltip class="dsc-tip">En proceso en el servidor...</q-tooltip>
                                </q-btn>

                                <q-btn v-else-if="props.row.estado === 'COMPLETADO'" dense round flat size="md"
                                    color="teal" icon="mdi-download" :loading="descargandoId === props.row.id"
                                    :disable="descargandoId !== null" @click="descargarArchivo(props.row)">
                                    <q-tooltip class="dsc-tip">Descargar archivo</q-tooltip>
                                </q-btn>

                                <q-btn v-else-if="props.row.estado === 'FALLIDO'" dense round flat size="md"
                                    color="red-6" icon="mdi-alert-circle-outline">
...
export default {
    name: 'DeclaracionAnualPage',
    data() {
        const hoy = new Date()
        return {
            tipoDocumento: 'anual_todas',
            endpointSolicitud: 'DescargarAnualTodas',
            ejercicio: String(hoy.getFullYear() - 1), 
            formato: 'ambos',
            catEjercicios: Array.from({ length: 10 }, (_, i) => String(hoy.getFullYear() - i)),
            catFormatos: [
                { label: 'PDF y Excel', value: 'ambos' },
                { label: 'Solo PDF', value: 'pdf' },
                { label: 'Solo Excel', value: 'excel' }
            ],
            ...
            columns: [
                ...
                { name: 'ejercicio', label: 'Ejercicio', field: 'ejercicio', align: 'center', sortable: true },
    ...
    methods: {
        ...
        // Descarga segura vía blob: fuerza content-type, sanitiza nombre, evita mixed content
        async descargarArchivo(item) {
            ...
            try {
                ...
                this.$q.notify({ type: 'negative', message: 'Error al descargar el archivo.' })
            } finally {
