    ...
    <div class="dsc-filters">
      <div class="dsc-filters__row">
        ...
        <div class="dsc-field dsc-field--sm">
          ...
          <q-select dense outlined v-model="anio" :options="catAnios" class="dsc-input" @input="onAnioMesChange" />
        </div>

        <!-- Mes -->
        <div class="dsc-field">
          <div class="dsc-field__label">Mes</div>
          <q-select dense outlined v-model="mes" :options="catMeses" option-label="label" class="dsc-input"
        ...
        <div class="dsc-field dsc-field--btns">
          <q-btn dense unelevated color="primary" icon="mdi-send" label="Solicitar" :loading="cargando"
            :disable="cargando || !mes || !anio" class="dsc-btn" @click="solicitar" />
          <q-btn dense unelevated outline color="primary" icon="mdi-refresh" label="Actualizar" :loading="cargando"
      ...
      <div v-if="descargandoId" class="dsc-alert dsc-alert--blue">
        ...
        <span>Descarga en progreso — no cierre la ventana ni repita la acción hasta
        ...
        <template v-slot:body="props">
          <q-tr :props="props" :class="rowClass(props.row)">
            <q-td key="fechaSolicitud" :props="props" class="dsc-td">{{
              formatFecha(props.row.fechaSolicitud)
            }}</q-td>
            ...
            <q-td key="estatusSolicitud" :props="props" class="dsc-td">
              <div v-if="descargandoId === props.row.solicitud" class="dsc-downloading">
                <q-spinner-dots size="1em" color="indigo" />&nbsp;
                Descargando...
              </div>
              <span v-else :class="['dsc-status', statusClass(props.row.estatusSolicitud)]">
                {{ displayEstatus(props.row.estatusSolicitud) }}
              </span>
            </q-td>
            ...
            <q-td key="acciones" :props="props" class="dsc-td" auto-width>
              <div class="dsc-actions">
                ...
                <q-btn v-if="props.row.estatusSolicitud === 'Descargando'" dense round flat size="md" color="blue-6"
                  icon="mdi-loading" disable loading>
                  <q-tooltip class="dsc-tip">Descarga en progreso en el servidor...</q-tooltip>
                </q-btn>

                <!-- Listo para descargar -->
                <q-btn v-if="props.row.estatusSolicitud === 'Listo para Descargar'" dense round flat size="md"
                  color="teal" icon="mdi-download" :loading="descargandoId === props.row.solicitud"
                ...
                " dense round flat size="md" :color="props.row.estatusSolicitud === 'Solicitud Realizada'
                      ? 'orange-8'
                      : 'blue-7'
                ...
                <template v-if="props.row.estatusSolicitud === 'Descargado'">
                  ...
                  <q-btn dense round flat size="md" color="grey-4" text-color="grey-6" icon="mdi-check-circle" disable>
                  ...
                  <q-btn dense round flat size="md" :color="props.row.conciliacionGuardada ? 'grey-5' : 'teal'"
                    icon="mdi-database-arrow-up" :loading="guardandoId === props.row.solicitud" :disable="props.row.conciliacionGuardada || guardandoId !== null
                      " @click="guardarConciliacion(props.row)">
                    <q-tooltip class="dsc-tip">
                      {{
                        props.row.conciliacionGuardada
                          ? "Ya guardado en conciliación"
                          : "Guardar en conciliación"
                      }}
                    </q-tooltip>
                  </q-btn>
...
export default {
  ...
  computed: {
    ...
    historial() {
      return this.$store.state.listaHistorialDescargasMetadataStore || [];
    },
    ...
    rangoTexto() {
      ...
      return (
        fi.format("DD/MM/YY HH:mm") + "  →  " + ff.format("DD/MM/YY HH:mm")
      );
    },
  },
  ...
  methods: {
    ...
    calcularRango() {
      ...
      this.fechaInicial =
        baseDate.startOf("month").format("YYYY-MM-DD") + " 00:00:00";
      ...
      if (esMesActual) {
        ...
        const primerDiaSiguiente = baseDate
          .clone()
          .add(1, "month")
          .startOf("month");
    ...
    async solicitar() {
      ...
      try {
        const res = await axios.post(
          this.ruta + "Descargas/PostSolicitud/erp_" + this.token.rfc,
          payload
        );
        ...
        this.$store.state.listaHistorialDescargasMetadataStore = arr.sort(
        ...
        this.$q.notify({
          type: "negative",
          message: (e.response && e.response.data) || "Error al solicitar.",
        });
      } finally {
        this.cargando = false;
        this.$q.loading.hide();
    ...
    async getHistorial() {
      ...
      try {
        const res = await axios.post(
          this.ruta +
          "Descargas/GetHistorialDescargas/Metadata/erp_" +
          this.token.rfc
        );
        this.$store.state.listaHistorialDescargasMetadataStore = (
          res.data || []
    ...
    async descargar(item) {
      ...
      this.$q.loading.show({
        message:
          "<b>Descargando...</b><br>Este proceso puede tardar varios minutos. No cierre la ventana.",
      });
      try {
        const res = await axios.post(
        ...
        item.estatusSolicitud =
          res.data.mensaje ===
            "Se han descargado satisfactoriamente las facturas."
            ? "Descargado"
            : res.data.mensaje;
        ...
        this.$q.notify({
          ...
          message: esTimeout
            ? "La descarga sigue en proceso en el servidor. El estatus se actualizará " +
            "automáticamente cada 2 minutos. No vuelvas a presionar el botón."
            : e.response?.data?.error || "Error al descargar.",
        });
      } finally {
    ...
    async guardarConciliacion(item) {
      ...
      try {
        ...
        this.$q.notify({
          type: "positive",
          message: "Guardado en conciliación.",
        });
      } catch (e) {
        this.$q.notify({
          type: "negative",
    ...
    async descargaLocal(item) {
      ...
      try {
        ...
        if (res.data.base64) {
          this.$refs.visor.abrir(
            res.data.base64,
            (item.solicitudPaquete || item.solicitud) + ".txt"
          );

          const blob = new Blob([atob(res.data.base64)], {
            type: "text/plain",
          });
    ...
    async ValidaCancelados(item) {
      ...
      try {
        await axios.post(
          this.ruta +
          "Descargas/PostValidaCanceladosAsync/" +
          item.rfc +
          "/" +
          item.solicitud +
          "/" +
    ...
    updateRow(item) {
      ...
      this.$store.state.listaHistorialDescargasMetadataStore = [...lista].sort(
        (a, b) => new Date(b.fechaSolicitud) - new Date(a.fechaSolicitud)
      );
    },
    ...
    displayEstatus(estatus) {
      if (
        estatus ===
        "Error inesperado en Send: SatServiceException - Error al descargar paquete. CodEstatus=404, Mensaje=Error no controlado."
      ) {
