      ...
      >
        <template v-slot:top>
          <div class="dsc-table__top">
            <span class="dsc-table__title"> Historial CFDI </span>
            <q-input
              dense
              outlined
              v-model="filter"
        ...
        <template v-slot:body="props">
          <q-tr :props="props" :class="rowClass(props.row)">
            ...
            <q-td key="tipoComprobante" :props="props" class="dsc-td">{{
...
export default {
  ...
  methods: {
    ...
    periodoValido() {
      ...
      if (
        this.token.nombre.toLowerCase() !== "admin" &&
    ...
    async solicitar() {
      ...
      try {
        ...
        this.$q.notify({
          type: "negative",
          message: e.response?.data || "Error al solicitar.",
        });
      } finally {
        this.cargando = false;
        this.$q.loading.hide();
    ...
    async getHistorial() {
      ...
      try {
        const res = await axios.post(
          this.ruta +
            "Descargas/GetHistorialDescargas/CFDI/erp_" +
            this.token.rfc
        );
        this.$store.state.listaHistorialDescargasCFDIStore = (
          res.data || []
    ...
    async descargar(item) {
      ...
      this.$q.loading.show({
        message:
          "<b>Descargando...</b><br>Este proceso puede tardar varios minutos.",
      });
      try {
        const res = await axios.post(
        ...
        item.estatusSolicitud =
          ...
          "Se han descargado satisfactoriamente las facturas."
            ? "Descargado"
            : res.data.mensaje;
        item.numComprobantesDescargados =
          res.data.numComprobantesDescargados || 0;
        this.updateRow("listaHistorialDescargasCFDIStore", item);
    ...
    _push(obj) {
      ...
      this.$store.state.listaHistorialDescargasCFDIStore = arr.sort(
        (a, b) => new Date(b.fechaSolicitud) - new Date(a.fechaSolicitud)
      );
    },
    updateRow(key, item) {
      ...
      this.$store.state[key] = [...lista].sort(
        (a, b) => new Date(b.fechaSolicitud) - new Date(a.fechaSolicitud)
      );
    },
    rowClass(row) {
