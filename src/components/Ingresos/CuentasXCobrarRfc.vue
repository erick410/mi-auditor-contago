<template>
    <div class="q-pa-md">
        ...
        <div class="row no-wrap items-center q-mt-md q-pa-sm">
            ...
            <q-select v-model="selectClientes" use-input input-debounce="0" label="Cliente por RFC"
                :options="optionsClientes" @filter="filterFn" fill-input hide-selected style="width: 300px"
        ...
        <q-table title="Reporte ISR" :data="dataComprobantes" :columns="columns" row-key="folioFiscal"
            ...
            <template v-slot:top>
                <span class="text-body1" content-style="font-size: 20px">Cuentas Por Cobrar por RFC</span>
                <q-space />
...
lista69
    ...
    methods: {
        ...
        async GetReporte() {
            ...
            try {
                let response = await axios.get(this.rutaAxios + 'Ingresos/GetCxCAsync/erp_' + this.token.rfc + '/' + fI + '/' + fF + '/' + this.selectClientes.value);
                let x = response.data;
           ...
           async GetGrafica() {
            ...
            const ordenados = [...objetos].sort((a, b) => b.porCobrar - a.porCobrar);

            const todos = ordenados.slice(0, 10);
            const importe = todos.map((item) => item.porCobrar);

            const sumaIngresos = this.dataComprobantes.reduce((acumulador, actual) => acumulador + actual.porCobrar, 0);
            const sumaTotal = sumaIngresos

            let ObjIngresos = {
                label: "Por Cobrar: " + this.FormatoMiles(sumaIngresos),
            ...
            let ObjIngresosLinea = {
                label: "Por Cobrar (Linea): " + this.FormatoMiles(sumaIngresos),
                type: 'line',
                borderColor: "rgba(0, 163, 92)",
                borderWidth: 1,
                data: importe,
            ...
            this.charTitleE = 'Cuentas por Cobrar por RFC: ' + this.FormatoMiles(sumaTotal)
        },

        async GetClientes(ObjFechaI, ObjFechaF) {
            this.OpenDialogLoading()
            try {
                let response = await axios.get(this.rutaAxios + 'Ingresos/GetClientesReporteAsync/erp_' + this.token.rfc + '/' + ObjFechaI + '/' + ObjFechaF);
                let x = response.data;
                this.options = [...x]
                this.$q.loading.hide()
            } catch (error) {
        ...
        //     XLSX.writeFile(workbook, 'CUENTAS POR COBRAR ' + this.selectClientes.value + ' DEL ' + fI + ' AL ' + fF + '.xlsx');
        // },

        ExportExcel() {
            let reporte = 'REPORTE DE CUENTAS POR COBRAR DE ' + this.selectClientes.value
