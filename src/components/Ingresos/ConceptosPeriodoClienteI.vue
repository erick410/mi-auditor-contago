...
<q-btn push color="red-10" @click="abrirDialogLista69B" icon="mdi-account-search" rounded flat
        ...
        <q-table title="Reporte ISR" :data="dataComprobantes" :columns="columns" row-key="folioFiscal"
            ...
            <template v-slot:top>
                <span class="text-body1" content-style="font-size: 20px">Conceptos por Cliente</span>
                <q-space />
  ...
  charTitleE: '',
    ...
    methods: {
        ...
        async GetReporte() {
            ...
            try {
                let response = await axios.get(this.rutaAxios + 'Ingresos/GetReporteConceptosClienteAsync/erp_' + this.token.rfc + '/' + fI + '/' + fF + '/' + this.selectClientes.value);
                let x = response.data;
        ...
        async GetClientes(ObjFechaI, ObjFechaF) {
            ...
            try {
                let response = await axios.get(this.rutaAxios + 'Ingresos/GetClientesTodosReporteAsync/erp_' + this.token.rfc + '/' + ObjFechaI + '/' + ObjFechaF);
                let x = response.data;
                this.options = [...x]
        ...
        //     XLSX.writeFile(workbook, 'CONCEPTOS FACTURADOS DEL ' + fI + ' AL ' + fF + ' POR CLIENTE.xlsx');
        // },

        ExportExcel() {
            let reporte = 'REPORTE DE CONCEPTOS FACTURADOS POR CLIENTE'
            ...
            xlsx.writeFile(
                workbook,
                rfc + ' - ' + empresa +  ' -REPORTE DE CONCEPTOS FACTURADOS POR CLIENTE DEL ' + periodo.toUpperCase() + '.xlsx'
            );
            },
