<template>
    <div class="q-pa-md">
        ...
        <!-- TABLA DE IVA -->
        <q-table title="Reporte ISR" :data="dataComprobantes" :columns="columns" :rows-per-page-options="[10]"
            :filter="filter">
            <template v-slot:top>
                <span class="text-body1" content-style="font-size: 20px">Comprobantes Facturados Sin Impuestos</span>
                <q-space />
...
dialogLista69B:false
    ...
    methods: {
        calcularTotales() {
            this.totalSubtotal = this.dataComprobantes.reduce((acumulador, objeto) => acumulador + objeto.subTotal, 0)
        ...
        async GetReporte() {
            ...
            try {
                let response = await axios.get(this.rutaAxios + 'Ingresos/GetReporteIngresosFacturadosSinImpuestosAsync/erp_' + this.token.rfc + '/' + fI + '/' + fF);
        ...
        //     XLSX.writeFile(workbook, 'COMPROBANTES FACTURADOS SIN IMPUESTOS DEL ' + fI + ' AL ' + fF + '.xlsx');
        // },

        ExportExcel() {
            let reporte = 'REPORTE DE COMPROBANTES FACTURADOS SIN IMPUESTOS'
            ...
            xlsx.writeFile(
                workbook,
                rfc + ' - ' + empresa +  ' - REPORTE DE COMPROBANTES FACTURADOS SIN IMPUESTOS DEL ' + periodo.toUpperCase() + '.xlsx'
            );
            },
