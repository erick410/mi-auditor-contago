<template>
    <div class="q-pa-md">
        ...
        <!-- DIALOG PARA SOLICITAR UNA CANCELACION -->
        <q-dialog v-model="dialogCancelacion" persistent transition-show="scale" transition-hide="scale" maximized>
            <cancelaciones @CloseDialogCancelacion="CloseDialogCancelacion"></cancelaciones>
        </q-dialog>
            ...
            <template v-slot:body="props">
                <q-tr :props="props">
                    ...
                    <q-td auto-width>
                        ...
                        <q-btn size="md" color="primary" rounded flat dense @click="ValidaArchivos(props.row)"
                            icon="mdi-cancel">
                            <q-tooltip transition-show="flip-right" transition-hide="flip-left"
...
import cancelaciones from '../Cancelados/SolicitudCancelacion.vue'
...
export default {
    ...
    methods: {
        async ValidaArchivos(item) {
            let archivos = await this.GetVigenciaArchivos();
            let banderaCsd = false;
            if (archivos) {
            ...
            if (banderaCsd) {
                console.log(item)
                this.$store.state.cancelacionStore = {
                    serie: item.serie,
                    folio: item.folio,
                    receptor: item.rfc + ' | ' + item.nombre,
                    folioFiscal: item.folioFiscal,
                };
                console.log('Podemos Cancelar')
        ...
        ExportExcel() {
            ...
            const cabecera = [
                ...
                ["RFC:", rfc.toUpperCase()],
                ["FOLIO FISCAL:",periodo.toUpperCase()],
            ...
            xlsx.writeFile(
                workbook,
                rfc + ' - ' + empresa +  ' - COMPLEMENTOS DE PAGO DE ' + periodo.toUpperCase() + '.xlsx'
            );
            },
