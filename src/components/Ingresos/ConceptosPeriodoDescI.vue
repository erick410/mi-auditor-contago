<template>
    ...
    <q-dialog v-model="dialogLista69B" persistent transition-show="scale" transition-hide="scale" maximized>
        ...
        <div class="row no-wrap items-center q-mt-md q-pa-sm">
            ...
            <q-input color="primary" v-model="descripcion" label="Descripción">
            </q-input>
        ...
        <q-table title="Reporte ISR" :data="dataComprobantes" :columns="columns" row-key="folioFiscal"
            ...
            <template v-slot:top>
                <span class="text-body1" content-style="font-size: 20px">Conceptos por Descripción</span>
                <q-space />
...
export default {
    ...
    methods: {
        ...
        async GetReporte() {
            ...
            try {
                let response = await axios.get(this.rutaAxios + 'Ingresos/GetReporteConceptosDescripcionAsync/erp_' + this.token.rfc + '/' + fI + '/' + fF + '/' + this.descripcion);
                let x = response.data;
        ...
        //     XLSX.writeFile(workbook, 'CONCEPTOS FACTURADOS DEL ' + fI + ' AL ' + fF + ' POR DESCRIPCIÓN.xlsx');
        // },

        ExportExcel() {
            let reporte = 'REPORTE DE CONCEPTOS FACTURADOS POR DESCRIPCIÓN'
            ...
            xlsx.writeFile(
                workbook,
                rfc + ' - ' + empresa +  ' - REPORTE DE CONCEPTOS FACTURADOS POR DESCRIPCIÓN DEL ' + periodo.toUpperCase() + '.xlsx'
            );
            },
