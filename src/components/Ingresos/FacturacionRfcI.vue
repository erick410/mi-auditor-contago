<template>
    ...
    <q-dialog v-model="dialogLista69B" persistent transition-show="scale" transition-hide="scale" maximized>
        ...
        <q-table title="Reporte ISR" :data="dataComprobantes" :columns="columns" row-key="rfc" :rows-per-page-options="[12]"
            ...
            <template v-slot:top>
                <span class="text-body1" content-style="font-size: 20px">Comprobantes Emitidos por RFC </span>
                <q-space />
...
export default {
    ...
    methods: {
        ...
        async GetReporte() {
            ...
            try {
                let response = await axios.get(this.rutaAxios + 'Ingresos/GetReporteAcumuladoPorRfcAsync/erp_' + this.token.rfc + '/' + fI + '/' + fF);
                let x = response.data;
                console.log(x)
        ...
        //     XLSX.writeFile(workbook, 'INGRESOS FACTURADOS DE ' + this.selectedMesI.label + ' A ' + this.selectedMesF.label + ' ' + this.selectedAnio + ' POR RFC.xlsx');
        // },

        ExportExcel() {
            let reporte = 'REPORTE DE INGRESOS FACTURADOS POR RFC'
            let empresa = this.$store.state.empresaStore.nombre
            let rfc = this.$store.state.empresaStore.rfc
           
            let periodo = this.selectedMesI.label + ' A ' + this.selectedMesF.label + ' ' + this.selectedAnio 
            ...
            xlsx.writeFile(
                workbook,
                rfc + ' - ' + empresa +  ' - REPORTE DE INGRESOS FACTURADOS POR RFC DE ' + periodo.toUpperCase() + '.xlsx'
            );
            },
