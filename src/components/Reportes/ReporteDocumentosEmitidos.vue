<template>
    <div class="q-pa-md">
        ...
        <q-table :data="dataComprobantes" :columns="columns" row-key="folioFiscal" :rows-per-page-options="[10]"
            title="Comprobantes Emitidos"  :loading="loading">
    ...
    methods: {
      async GetReporte() {
        this.loading = true
            ...
            try {
                let response = await axios.get(this.rutaAxios + 'Ingresos/GetReporteDocumentosEmitidosAsync/erp_' + this.token.rfc + '/' + fI + '/' + fF );
                let x = response.data;
                console.log(x)
...
XLSX.utils.book_append_sheet(wb, ws, 'Datos');
            XLSX.writeFile(wb,  rfc + ' - ' + empresa + ' - COMPROBANTES EMITIDOS ' + fI + ' AL ' + fF + '.xlsx');
        },
         UltimoDiaMes() {
