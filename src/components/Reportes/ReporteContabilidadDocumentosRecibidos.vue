<template>
    <div class="q-pa-md">
        ...
        <q-table :data="dataComprobantes" :columns="columns" row-key="folioFiscal" :rows-per-page-options="[10]"
            title="Contabilidad Documentos Recibidos"  :loading="loading">
    ...
    methods: {
      async GetReporte() {
        this.loading = true
            ...
            try {
                let response = await axios.get(this.rutaAxios + 'Ingresos/GetReporteDocumentosContabilidadRecibidosAsync/erp_' + this.token.rfc + '/' + fI + '/' + fF );
                let x = response.data;
                console.log(x)
        ...
        ExportExcel() {
            ...
            XLSX.writeFile(wb,  rfc + ' - ' + empresa + ' - CONTABILIDAD COMPROBANTES RECIBIDOS ' + fI + ' AL ' + fF + '.xlsx');
        },
         UltimoDiaMes() {
