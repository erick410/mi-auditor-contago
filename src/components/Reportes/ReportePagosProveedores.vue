...
<!-- TABLA DE COMPLEMENTOS DE PAGO -->
    <q-table :data="dataComprobantes" :columns="columns" row-key="folioFiscal" :rows-per-page-options="[10]"
            title="Pagos Proveedores"  :loading="loading">
    ...
    data() {
        return {
            ...
            columns: [
                ...
                { name: 'rfc', align: 'left', label: 'RFC del Proveedor', field: 'rfc', sortable: true },
                { name: 'nombre', align: 'left', label: 'Razon social del Proveedor', field: 'nombre', sortable: true },
    ...
    methods: {
      async GetReporte() {
        this.loading = true
            ...
            try {
                let response = await axios.get(this.rutaAxios + 'Gastos/GetReportePagosProveedoresAsync/erp_' + this.token.rfc + '/' + fI + '/' + fF);
                let x = response.data;
        ...
        ExportExcel() {
            ...
            XLSX.writeFile(wb, rfc + ' - ' + empresa + ' - PAGOS A PROVEEDORES DEL ' + fI + ' AL ' + fF + '.xlsx');
        },
    },
}
</script>