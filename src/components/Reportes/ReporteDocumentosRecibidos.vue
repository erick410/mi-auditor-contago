<template>
    <div class="q-pa-md">
        ...
        <q-table :data="dataComprobantes" :columns="columns" row-key="folioFiscal" :rows-per-page-options="[10]"
            title="Comprobantes Recibidos"  :loading="loading">
        ...
        <q-inner-loading showing color="primary" />
            ...
            <template v-slot:body="props">
                <q-tr :props="props">
                    ...
                    <q-td key="tipoProveedor" :props="props"></q-td>
    ...
    data() {
        return {
            ...
            columns: [
                ...
                { name: 'tipoProveedor', align: 'left', label: 'Tipo Proveedor', field: 'tipoProveedor', sortable: true },
                { name: 'rfcReceptor', align: 'left', label: 'RFC Proveedor', field: 'rfcReceptor', sortable: true },
                { name: 'nombreReceptor', align: 'left', label: 'Razon social del Proveedor', field: 'nombreReceptor', sortable: true },
    ...
    methods: {
      async GetReporte() {
        this.loading = true
            ...
            try {
                let response = await axios.get(this.rutaAxios + 'Ingresos/GetReporteDocumentosRecibidosAsync/erp_' + this.token.rfc + '/' + fI + '/' + fF );
                let x = response.data;
                console.log(x)
        ...
        ExportExcel() {
            ...
            XLSX.writeFile(wb,  rfc + ' - ' + empresa + ' - COMPROBANTES RECIBIDOS ' + fI + ' AL ' + fF + '.xlsx');
        },
         UltimoDiaMes() {
