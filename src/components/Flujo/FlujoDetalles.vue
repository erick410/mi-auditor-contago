        ...
        <q-table title="Reporte ISR" :data="comprobante.detalles" :columns="columns" row-key="rfc"
            ...
            <template v-slot:body="props">
                <q-tr :props="props">
                    ...
                    <q-td key="totalCobro" :props="props">{{ FormatCurrency(props.row.totalCobro) }}</q-td>
                    ...
                    <q-td key="totalCobroPesos" :props="props">{{ FormatCurrency(props.row.totalCobroPesos) }}</q-td>
...
export default {
    ...
    data() {
        return {
            columns: [
                ...
                { name: 'totalCobro', align: 'right', label: 'Total Cobrado', field: 'totalCobro', sortable: true },
                ...
                { name: 'totalCobroPesos', align: 'right', label: 'Total Cobrado Pesos', field: 'totalCobroPesos', sortable: true },
    ...
    methods: {
        ...
        ExportExcel() {
            let reporte = 'REPORTE DE FLUJO DETALLADO'
            ...
            xlsx.writeFile(
                workbook,
                rfc + ' - ' + empresa +  ' - REPORTE DE FLUJO DETALLADO DEL ' + periodo.toUpperCase() + '.xlsx'
            );
            },
        ...
        async VerComprobanteI(item) {
            try {
                console.log(item)
                let response = await axios.get(this.rutaAxios + 'Comprobante/GetComprobanteAsync/erp_' + this.token.rfc + '/' + item.folioFiscalComprobante);
                let x = response.data;
                console.log(x);
                let color = "#" + '00A35C';
        ...
        async VerComprobanteP(item) {
            try {
                console.log(item)
                let response = await axios.get(this.rutaAxios + 'Comprobante/GetComprobantePagoEmitidoAsync/erp_' + this.token.rfc + '/' + item.folioFiscalPago);
                let x = response.data;
                console.log(x);
                let color = "#" + '00A35C';
