<template>
    <div class="q-pa-md">
        ...
        <q-table title="Reporte ISR" :data="dataComprobantes" :columns="columns" row-key="rfc" :rows-per-page-options="[10]"
            ...
            <template v-slot:top>
                <span class="text-body1" content-style="font-size: 20px">Ingresos Cobrados {{ FormatCurrency(sumaPesos) }}</span>
                <q-space />
...
import detalles from './FlujoDetalles'
...
export default {
    ...
    methods: {
        async GetReporte() {
            ...
            try {
                let response = await axios.get(this.rutaAxios + 'Ingresos/GetFlujoAsync/erp_' + this.token.rfc + '/' + fI + '/' + fF);
                let x = response.data;
        ...
        ExportExcel() {
            let reporte = 'REPORTE DE FLUJO DE INGRESOS'
            ...
            xlsx.writeFile(
                workbook,
                rfc + ' - ' + empresa +  ' - REPORTE DE FLUJO DE INGRESOS DEL ' + periodo.toUpperCase() + '.xlsx'
            );
            },
