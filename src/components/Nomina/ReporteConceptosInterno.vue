<template>
    <div class="q-pa-md">
        ...
        <q-table title="Reporte ISR" :data="dataConceptos" :columns="columns" row-key="rfc" :rows-per-page-options="[10]">
            <template v-slot:top>
                <span class="text-body1" content-style="font-size: 20px">Conceptos Catálogo Interno</span>
                <q-space />
...
export default {
    ...
    methods: {
        ...
        async GetReporte() {
            ...
            try {
                let response = await axios.get(this.rutaAxios + 'Nomina/GetReporteConceptosInternoAsync/erp_' + this.token.rfc + '/' + fI + '/' + fF);
                let x = response.data;
        ...
        //     XLSX.writeFile(workbook, 'Reporte conceptos internos del ' + fI + ' al ' + fF + '.xlsx');
        // },

        ExportExcel() {
            const nuevaLista = this.dataConceptos;
            let reporte = 'REPORTE DE CONCEPTOS INTERNOS'
            ...
            const dataExcel =nuevaLista.map(row => {
                const obj = {};
            ...
            xlsx.writeFile(
                workbook,
                rfc + ' - ' + empresa +  ' - REPORTE DE CONCEPTOS INTERNOS DEL ' + periodo.toUpperCase() + '.xlsx'
            );
            },
